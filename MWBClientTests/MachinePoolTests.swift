...
final class MachinePoolTests: XCTestCase {
    ...
    func testUpdateMachineMatrixFromFourPackets() {
        ...
        for i in 0..<4 {
            let src = MachineID(rawValue: UInt32(i + 1))
    ...
    func testMatrixIndexIsOneBasedFromSrc() {
        ...
        pool.updateMachineMatrix(packetType: MatrixFlags.matrix, src: MachineID(rawValue: 2), machineName: "Second")
    ...
    func testInvalidSrcRejected() {
        ...
        pool.updateMachineMatrix(packetType: MatrixFlags.matrix, src: .none, machineName: "Zero")
        pool.updateMachineMatrix(packetType: MatrixFlags.matrix, src: MachineID(rawValue: 5), machineName: "Five")
    ...
    func testSendMachineMatrixProducesFourPackets() {
        ...
        for (i, pkt) in packets.enumerated() {
            XCTAssertEqual(pkt.src, MachineID(rawValue: UInt32(i + 1)))
...
extension MachinePoolTests {
    func testMatrixUpdatesApplyImmediately() {
        let pool = MachinePool()
        pool.updateMachineMatrix(packetType: 128, src: MachineID(rawValue: 1), machineName: "Win1")
        pool.updateMachineMatrix(packetType: 128, src: MachineID(rawValue: 2), machineName: "Win2")
